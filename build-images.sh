...
# Reuse existing nodebuilder-zammad container, to speed up builds
if ! buildah containers --format "{{.ContainerName}}" | grep -q nodebuilder-zammad; then
    echo "Pulling NodeJS runtime..."
    buildah from --name nodebuilder-zammad -v "${PWD}:/usr/src:Z" docker.io/library/node:lts
fi
...
buildah config --entrypoint=/ \
    ...
    --label="org.nethserver.images=docker.io/postgres:15.5-alpine3.19 docker.io/zammad/zammad-docker-compose:6.3.0 docker.io/bitnami/elasticsearch:8.12.2 ghcr.io/zammad/zammad:6.3.0 docker.io/memcached:1.6.25-alpine docker.io/redis:7.2.4-alpine" \
    "${container}"
# Commit the image
